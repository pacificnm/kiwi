# Kiwi Memory MCP Setup

For the full setup guide, including Cursor configuration, see
[`MCP-SETUP.md`](MCP-SETUP.md).

This document describes the local setup for the Kiwi project-memory MCP
...
Run this SQL in the `kiwi_memory` database:
...
```json
{
  "mcpServers": {
    "kiwi-memory": {
      "command": "/home/jaimie/kiwi/.venv/bin/python",
      "args": [
        "/home/jaimie/kiwi/tools/mcp_memory_server.py"
      ],
      "env": {
        "DATABASE_URL": "postgresql:///kiwi_memory?host=/var/run/postgresql",
        "OPENAI_API_KEY": "sk-..."
...
order in `AGENTS.md`: `docs/specs/` first, then MCP project memory, then
...
| --- | --- | --- |
| `Missing Python dependency for Kiwi memory MCP` | The MCP server is not running inside the virtual environment, or dependencies are not installed. | Run with `.venv/bin/python` and install `openai psycopg "mcp[cli]"`. |
...
| `type "vector" does not exist` | `pgvector` is not installed or `CREATE EXTENSION vector` has not been run. | Install the PostgreSQL `pgvector` package and enable the extension in `kiwi_memory`. |
...
| Empty search results | The database is empty. | Run `./scripts/index-memory.sh`. |
| `permission denied for schema public` during context setup | The app user can write rows but cannot create tables. | Run `sudo -u postgres psql kiwi_memory -f tools/setup_context_memory.sql`. |
