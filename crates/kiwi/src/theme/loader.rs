...
pub fn load_theme(settings: &ThemeSettings) -> Result<ThemePalette, ThemeError> {
    ...
    load_theme_with_capabilities(settings, capabilities)
}
...
fn resolve_definition(
    ...
    for role in SemanticRole::ALL {
        ...
        roles.insert(
            role,
            role_to_style(role, color.to_ratatui_color(capabilities)),
        );
    }
...
fn role_to_style(role: SemanticRole, color: ratatui::style::Color) -> Style {
    match role {
        SemanticRole::Bg => Style::default().bg(color),
        SemanticRole::Selection => Style::default().bg(color),
