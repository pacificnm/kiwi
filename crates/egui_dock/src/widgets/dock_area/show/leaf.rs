use egui::{
    emath::TSTransform, epaint::TextShape, lerp, pos2, vec2, Align, Align2, Color32,
...
impl<Tab> DockArea<'_, Tab> {
    ...
    fn tab_bar(
        ...
        let show_leaf_collapse = self.show_leaf_collapse_buttons && {
            if let Node::Leaf { tabs, .. } = &mut self.dock_state[surface_index][node_index] {
                tabs.iter_mut().any(|tab| tab_viewer.collapseable(tab))
            } else {
                false
            }
        };
        ...
        for tab_index in 0..tabs_len {
            ...
            let (response, title_id) = if is_being_dragged {
                ...
                if self.tab_context_menus {
                    let eject_label =
                        self.dock_state.translations.tab_context_menu.eject_button.clone();
                    let close_label =
                        self.dock_state.translations.tab_context_menu.close_button.clone();
                    ...
                    let tab = &mut tabs[tab_index.0];
                    let show_eject = (surface_index.is_main() || !is_lonely_tab)
                        && tab_viewer.allowed_in_windows(tab);

                    response.context_menu(|ui| {
                        let menu = tab_viewer.tab_context_menu(
                            ui,
                            tab,
                            surface_index,
                            node_index,
                            crate::widgets::tab_viewer::TabContextMenuOptions {
                                eject_label: &eject_label,
