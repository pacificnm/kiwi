...
export async function fetchAppMetadata(): Promise<AppMetadata> {
  return nestInvoke<AppMetadata>("nest_app_metadata");
}

export async function fetchThemeCss(): Promise<ThemeCss> {
  return nestInvoke<ThemeCss>("nest_theme_css");
}
...
export async function fetchImage(
  ...
  return nestInvoke<ImageFetchResponse>("nest_image_fetch", {
...
export async function invalidateImageTag(tag: string): Promise<{ removed_count: number }> {
  return nestInvoke<{ removed_count: number }>("nest_image_invalidate_tag", {
    request: { tag },
  });
}
